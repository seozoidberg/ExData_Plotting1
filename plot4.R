...
png(file = "plot4.png")
  par(mfrow = c(2,2))
    #### 4.1
    plot(my_subset_data$fulltime, my_subset_data$Global_active_power, type = "l", xlab = "", ylab ="Global Active Power")                                   #### <<<-----
    
    #### 4.2  
    plot(my_subset_data$fulltime, my_subset_data$Voltage, type = "l", xlab = "datetime", ylab ="Voltage")         
    ...
    plot(my_subset_data$fulltime, my_subset_data$Sub_metering_1, type = "n", xlab = "", ylab ="Energy sub metering")
      ...
      lines(my_subset_data$fulltime, my_subset_data$Sub_metering_3, col = "blue")
        legend("topright", legend = names(my_subset_data)[7:9], lty = 1, col = c("black", "red", "blue"), bty = "n")
    
    #### 4.4
    plot(my_subset_data$fulltime, my_subset_data$Global_reactive_power, type = "l", xlab = "datetime", ylab ="Global_reactive_power") 
dev.off()